...
def _loader(cfg: dict[str, Any], dataset: AEGDataset, *, train: bool) -> DataLoader:
    ...
    # Compact-KL consistency does not require in-batch negatives.
    # Therefore we do not drop the last small batch.
    drop_last = False
...
def train_one_epoch(
    ...
    model.train()
    loss_mode = str((cfg.get("loss", {}) or {}).get("mode", "compact_kl")).lower()
    use_aug = (
        bool((cfg.get("robust", {}) or {}).get("train_aug", True))
        and loss_mode in {"plain_kl", "compact_kl"}
    )
