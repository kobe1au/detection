...
class PrototypeMemory(nn.Module):
    def __init__(self, num_classes: int, feature_dim: int, momentum: float = 0.99):
        ...
        self.register_buffer("prototypes", F.normalize(torch.randn(self.num_classes, self.feature_dim), dim=-1))
    ...
    def update(self, feats: torch.Tensor, labels: torch.Tensor):
        ...
        for c in labels.unique():
            ...
            if self.seen[c] == 0:
                self.prototypes[c] = center.to(self.prototypes.device, self.prototypes.dtype)
            else:
                old = self.prototypes[c]
                new = center.to(old.device, old.dtype)
                self.prototypes[c] = F.normalize(self.momentum * old + (1.0 - self.momentum) * new, dim=-1)
            self.seen[c] += int(mask.sum().item())

    @torch.no_grad()
    def update_weighted(self, feats: torch.Tensor, labels: torch.Tensor, weights: torch.Tensor):
        if feats is None or labels is None or feats.numel() == 0:
            return
        feats = F.normalize(feats.detach().float(), dim=-1)
        labels = labels.detach().long().view(-1)
        ...
        for c in labels.unique():
            ...
            if w.sum() <= 1e-8:
                continue
            center = F.normalize(((w / w.sum().clamp_min(1e-8)) * feats[mask]).sum(dim=0), dim=-1)
            if self.seen[c] == 0:
                self.prototypes[c] = center.to(self.prototypes.device, self.prototypes.dtype)
            else:
                old = self.prototypes[c]
    ...
    def get_loss_quality_gated(self, feats: torch.Tensor, labels: torch.Tensor, quality_weights: Optional[torch.Tensor] = None) -> torch.Tensor:
        if feats is None or labels is None or feats.numel() == 0:
            dev = feats.device if feats is not None else (labels.device if labels is not None else torch.device("cpu"))
        ...
        loss = 1.0 - F.cosine_similarity(feats_seen, proto, dim=-1)
        if quality_weights is not None:
            q = quality_weights.float().view(-1).to(feats.device)[seen_mask]
            loss = loss * (q / q.mean().clamp_min(1e-8))
        return loss.mean()


class TemporalPrototypeMemory(nn.Module):
    def __init__(self, num_domains: int, num_classes: int, feature_dim: int, momentum: float = 0.99, num_clusters: int = 4):
        super().__init__()
        ...
        shape = (self.num_domains, self.num_classes, self.num_clusters)
        proto = F.normalize(torch.randn(*shape, self.feature_dim), dim=-1)
        self.register_buffer("prototypes", proto)
        self.register_buffer("seen", torch.zeros(shape, dtype=torch.long))
        self.register_buffer("spread", torch.zeros(shape, dtype=torch.float32))
        self.register_buffer("initialized", torch.zeros(shape, dtype=torch.bool))
        self.register_buffer("inherited", torch.zeros(shape, dtype=torch.bool))
    ...
    def occupancy_stats(self) -> dict[str, float]:
        observed = self.seen > 0
        initialized = self.initialized | observed
        observed_cells = observed.any(dim=2)
        initialized_cells = initialized.any(dim=2)
        mean_seen = 0.0
        if observed_cells.any():
            mean_seen = float(observed.sum(dim=2)[observed_cells].float().mean().item())
        mean_init = 0.0
        if initialized_cells.any():
            mean_init = float(initialized.sum(dim=2)[initialized_cells].float().mean().item())
        return {
            "occupied_cells": float(observed_cells.sum().item()),
            "total_cells": float(self.num_domains * self.num_classes),
            "occupied_clusters": float(observed.sum().item()),
            "mean_clusters_per_seen_cell": mean_seen,
            "initialized_cells": float(initialized_cells.sum().item()),
            "initialized_clusters": float(initialized.sum().item()),
            "mean_initialized_clusters_per_cell": mean_init,
        }

    def _valid_label_time_mask(self, labels: torch.Tensor, time_ids: torch.Tensor) -> torch.Tensor:
        return (labels >= 0) & (labels < self.num_classes) & (time_ids >= 0) & (time_ids < self.num_domains)
    ...
    def _select_diverse_centers(self, feats: torch.Tensor, weights: torch.Tensor, existing: torch.Tensor, count: int) -> torch.Tensor:
    ...
    @torch.no_grad()
    def update_weighted(self, feats: torch.Tensor, labels: torch.Tensor, time_ids: torch.Tensor, weights: Optional[torch.Tensor] = None):
        ...
        if not valid.any():
            return
        feats, labels, time_ids, weights = feats[valid], labels[valid], time_ids[valid], weights[valid]
        ...
        for ck in compound_key.unique():
            ...
            if group_weights.sum() <= 1e-8:
                continue
            observed = self.seen[dom_i, c_i] > 0
            assignable = self.initialized[dom_i, c_i] | observed
            unseen_idx = torch.where(~assignable)[0]
            ...
            num_new = max(0, min(int(unseen_idx.numel()), target_clusters - int(assignable.sum().item())))
            if num_new > 0:
                carried = 0
                for k in unseen_idx:
                    if carried >= num_new:
                    ...
                    if prev_dom is None:
                        continue
                    self.prototypes[dom_i, c_i, k_i] = self.prototypes[prev_dom, c_i, k_i].to(self.prototypes.device, self.prototypes.dtype)
                    self.spread[dom_i, c_i, k_i] = self.spread[prev_dom, c_i, k_i].to(self.spread.device, self.spread.dtype)
                    self.initialized[dom_i, c_i, k_i] = True
                    self.inherited[dom_i, c_i, k_i] = True
                    carried += 1
                assignable = self.initialized[dom_i, c_i] | (self.seen[dom_i, c_i] > 0)
                remaining = num_new - carried
                unseen_idx = torch.where(~assignable)[0]
                existing = self.prototypes[dom_i, c_i, assignable]
                new_centers = group_feats[:0] if remaining <= 0 else self._select_diverse_centers(group_feats, group_weights, existing, remaining)
                for j, center in enumerate(new_centers):
                    k_i = int(unseen_idx[j].item())
                    self.prototypes[dom_i, c_i, k_i] = F.normalize(center, dim=-1).to(self.prototypes.device, self.prototypes.dtype)
                    self.initialized[dom_i, c_i, k_i] = True
                    self.inherited[dom_i, c_i, k_i] = False
            assignable = self.initialized[dom_i, c_i] | (self.seen[dom_i, c_i] > 0)
            if not assignable.any():
                continue
            cluster_ids = torch.where(assignable)[0]
            cluster_proto = self.prototypes[dom_i, c_i, cluster_ids].to(group_feats.device, group_feats.dtype)
            assigned = cluster_ids.to(group_feats.device)[torch.matmul(group_feats, cluster_proto.t()).argmax(dim=1)]
            for k in assigned.unique():
                ...
                batch_spread = (self._normalize_weights(group_weights[cmask]).to(sample_spread.device) * sample_spread).sum().to(self.spread.device, self.spread.dtype)
                new_count = int(cmask.sum().item())
                if old_count <= 0:
                    self.prototypes[dom_i, c_i, k_i] = new
                    self.spread[dom_i, c_i, k_i] = batch_spread
                else:
                    total_count = max(old_count + new_count, 1)
                    old_spread = self.spread[dom_i, c_i, k_i]
                    self.spread[dom_i, c_i, k_i] = (old_spread * float(old_count) + batch_spread * float(new_count)) / float(total_count)
                    self.prototypes[dom_i, c_i, k_i] = F.normalize(self.momentum * old + (1.0 - self.momentum) * new, dim=-1).to(old.device, old.dtype)
                self.initialized[dom_i, c_i, k_i] = True
                self.inherited[dom_i, c_i, k_i] = False
                self.seen[dom_i, c_i, k_i] += new_count

    def get_loss_quality_gated(self, feats: torch.Tensor, labels: torch.Tensor, time_ids: torch.Tensor, quality_weights: Optional[torch.Tensor] = None) -> torch.Tensor:
        if feats is None or labels is None or time_ids is None or feats.numel() == 0:
            dev = feats.device if feats is not None else (labels.device if labels is not None else torch.device("cpu"))
            dt = feats.dtype if feats is not None else torch.float32
            return torch.tensor(0.0, device=dev, dtype=dt)
        ...
        cluster_valid = (self.seen[time_ids, safe_labels] > 0).to(feats.device)
        ...
        all_proto = self.prototypes[seen_time_ids].to(feats.device, feats.dtype)
        all_valid = (self.seen[seen_time_ids] > 0).to(feats.device)
        ...
        if quality_weights is not None:
            ...
            if q_all.numel() == feats.size(0):
                q = q_all[seen_mask]
                base_loss = base_loss * (q / q.mean().clamp_min(1e-8))
        return base_loss.mean()

    def forecast_next_prototypes(self, time_ids: torch.Tensor, velocity_scale: float = 1.0, min_history: int = 2) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if int(min_history) <= 1:
            ...
            valid = current_seen.clone()
        else:
            valid = has_trajectory.clone()
        ...
        for c in range(self.num_classes):
            for k in range(self.num_clusters):
                ...
                if not future_rows.any():
                    continue
                gap = (domain_ids[future_rows] - latest).to(self.prototypes.device, self.prototypes.dtype).view(-1, 1)
                cls_velocity = self.prototypes[latest, c, k] - self.prototypes[prev, c, k]
                future[future_rows, c, k] = F.normalize(self.prototypes[latest, c, k].view(1, -1) + float(velocity_scale) * gap * cls_velocity.view(1, -1), dim=-1)
                valid[future_rows, c, k] = True
        return future, valid

    def get_future_forecast_loss(self, feats: torch.Tensor, labels: torch.Tensor, time_ids: torch.Tensor, quality_weights: Optional[torch.Tensor] = None, temperature: float = 0.2, velocity_scale: float = 1.0, min_history: int = 2) -> torch.Tensor:
        if feats is None or labels is None or time_ids is None or feats.numel() == 0:
            dev = feats.device if feats is not None else (labels.device if labels is not None else torch.device("cpu"))
            dt = feats.dtype if feats is not None else torch.float32
            return torch.tensor(0.0, device=dev, dtype=dt)
        ...
        future_proto, valid_cls = self.forecast_next_prototypes(time_ids, velocity_scale=velocity_scale, min_history=min_history)
        future_proto = future_proto.to(z.device, z.dtype)
        valid_cls = valid_cls.to(z.device)
        if future_proto.shape[:3] != (z.size(0), self.num_classes, self.num_clusters):
            return z.new_tensor(0.0)
        safe_labels = labels.clamp(0, self.num_classes - 1)
        ...
        logits = logits.masked_fill(~valid_cls, -1e4).max(dim=2).values.masked_fill(~valid_class, -1e4)
        per_sample = F.cross_entropy(logits[valid_row], safe_labels[valid_row], reduction="none")
        ...
        if has_negative.any():
            ...
            per_sample = per_sample + self.future_rank_weight * F.relu(neg_sim - pos_sim + self.future_rank_margin) * has_negative.to(z.dtype)
        if quality_weights is not None:
            q = quality_weights.float().view(-1).to(z.device)
            if q.numel() == z.size(0):
                q = q[valid_row].clamp(0.0, 1.0)
                per_sample = per_sample * (q / q.mean().clamp_min(1e-8)).detach()
        return per_sample.mean()

    def _nearest_class_distances(self, z: torch.Tensor, time_ids: torch.Tensor, include_future: bool = True, velocity_scale: float = 0.5, min_history: int = 2) -> tuple[torch.Tensor, torch.Tensor]:
        proto = self.prototypes[time_ids].to(z.device, z.dtype)
        valid = (self.seen[time_ids] > 0).to(z.device)
        ...
        if include_future:
            future_proto, future_valid = self.forecast_next_prototypes(time_ids, velocity_scale=velocity_scale, min_history=min_history)
            if future_proto.shape[:3] == (z.size(0), self.num_classes, self.num_clusters):
                future_proto = future_proto.to(z.device, z.dtype)
                future_valid = future_valid.to(z.device)
                future_cos = torch.einsum("bd,bckd->bck", z, future_proto).clamp(-1.0, 1.0)
                future_dist = ((1.0 - future_cos) * 0.5).clamp(0.0, 1.0).masked_fill(~future_valid, 1.0).min(dim=2).values
    ...
    def _cluster_reliability(self, time_ids: torch.Tensor, device: torch.device, dtype: torch.dtype) -> tuple[torch.Tensor, torch.Tensor]:
        domain_ids = time_ids.long().view(-1).clamp(0, self.num_domains - 1)
        if domain_ids.numel() == 0:
            empty = torch.zeros((0, self.num_classes, self.num_clusters), device=device, dtype=dtype)
    ...
    def estimate_drift(self, feats: torch.Tensor, time_ids: torch.Tensor, class_probs: Optional[torch.Tensor] = None, labels: Optional[torch.Tensor] = None, include_future: bool = True, velocity_scale: float = 0.5, min_history: int = 2) -> torch.Tensor:
        if feats is None or time_ids is None or feats.numel() == 0:
            dev = feats.device if feats is not None else (time_ids.device if time_ids is not None else torch.device("cpu"))
            return torch.zeros((0, 1), device=dev, dtype=torch.float32)
        z = F.normalize(feats.float(), dim=-1)
        time_ids = time_ids.long().view(-1).clamp(0, self.num_domains - 1)
        if time_ids.numel() != z.size(0):
            return z.new_zeros((z.size(0), 1))
        class_dist, valid_class = self._nearest_class_distances(z, time_ids, include_future, velocity_scale, min_history)
        if labels is not None:
            ...
            valid_label = (labels >= 0) & (labels < self.num_classes)
            row = torch.arange(z.size(0), device=z.device)
            has_current = valid_label & valid_class[row, safe_labels]
            score = z.new_zeros((z.size(0),))
            if has_current.any():
                score[has_current] = class_dist[row[has_current], safe_labels[has_current]]
        else:
            if class_probs is None:
                class_probs = torch.full((z.size(0), self.num_classes), 1.0 / float(self.num_classes), device=z.device, dtype=z.dtype)
            else:
            ...
            score = (class_dist * weights).sum(dim=1) / weights.sum(dim=1).clamp_min(1e-8)
        score = torch.where(has_current, score, torch.zeros_like(score))
        return score.view(-1, 1).clamp(0.0, 1.0)

    def estimate_risk_components(self, feats: torch.Tensor, time_ids: torch.Tensor, class_probs: Optional[torch.Tensor] = None, include_future: bool = True, velocity_scale: float = 0.5, min_history: int = 2) -> dict[str, torch.Tensor]:
        if feats is None or time_ids is None or feats.numel() == 0:
            dev = feats.device if feats is not None else (time_ids.device if time_ids is not None else torch.device("cpu"))
            empty = torch.zeros((0, 1), device=dev, dtype=torch.float32)
            return {"temporal_drift": empty, "prototype_pred_distance": empty, "prototype_margin_risk": empty, "prototype_label_mismatch": empty, "prototype_reliability_risk": empty}
        z = F.normalize(feats.float(), dim=-1)
        time_ids = time_ids.long().view(-1).clamp(0, self.num_domains - 1)
        if time_ids.numel() != z.size(0):
            zeros = z.new_zeros((z.size(0), 1))
            return {"temporal_drift": zeros, "prototype_pred_distance": zeros, "prototype_margin_risk": zeros, "prototype_label_mismatch": zeros, "prototype_reliability_risk": zeros}
        class_dist, valid_class = self._nearest_class_distances(z, time_ids, include_future, velocity_scale, min_history)
        cluster_reliability, reliability_valid = self._cluster_reliability(time_ids, z.device, z.dtype)
        if class_probs is None:
            class_probs = torch.full((z.size(0), self.num_classes), 1.0 / float(self.num_classes), device=z.device, dtype=z.dtype)
        else:
            class_probs = class_probs.float().to(z.device)
        ...
        pred_dist[pred_valid] = class_dist[row[pred_valid], pred_label[pred_valid]]
        weights = class_probs.clamp_min(0.0) * valid_class.to(z.dtype)
        ...
        second_dist = sorted_dist[:, 1] if self.num_classes > 1 else torch.ones_like(best_dist)
        ...
        label_mismatch = ((pred_label != best_label) & pred_valid & has_any).to(z.dtype)
        ...
        pred_reliability[pred_valid] = class_reliability[row[pred_valid], pred_label[pred_valid]]
        reliability_risk = torch.where(has_any, 1.0 - pred_reliability, torch.zeros_like(pred_reliability)).clamp(0.0, 1.0)
